...
train_folders = '/cptjack/totem/yanyiting/gray_focus_unfocus/gray/data/2_focus/train'
validation_folders = '/cptjack/totem/yanyiting/gray_focus_unfocus/gray/data/2_focus/val'
...
def get_callbacks(filepath,model,j,patience=8):
    ...
    msave = Mycbk(model, './Resnet50_gray_jingxi'+str(j)+'/'+filepath)
    file_dir = './Resnet50_gray_jingxi'+str(j)+'/'+'log/'+ time.strftime('%Y_%m_%d',time.localtime(time.time()))
    ...
    log_cv = CSVLogger('./Resnet50_gray_jingxi'+str(j)+'/' + time.strftime('%Y_%m_%d',time.localtime(time.time()))  +'_log.csv', separator=',', append=True)
