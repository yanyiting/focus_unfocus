...
train_folders = '/cptjack/totem/yanyiting/gray_focus_unfocus/gray/data/gray/8_focus/val/'
validation_folders = '/cptjack/totem/yanyiting/gray_focus_unfocus/gray/data/gray/8_focus/train/'
...
def get_callbacks(filepath,model,j,patience=8):
    ...
    msave = Mycbk(model, './Resnet50_gray_2'+str(j)+'/'+filepath)
    file_dir = './Resnet50_gray_2'+str(j)+'/'+'log/'+ time.strftime('%Y_%m_%d',time.localtime(time.time()))
    ...
    log_cv = CSVLogger('./Resnet50_gray_2'+str(j)+'/' + time.strftime('%Y_%m_%d',time.localtime(time.time()))  +'_log.csv', separator=',', append=True)
