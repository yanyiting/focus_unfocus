...
train_folders = '/cptjack/totem/yanyiting/Eight_classification/gray/8_focus/val'
label_dir = os.listdir(train_folders)
for i in range(len(label_dir)):
    focus_pngs = glob.glob(train_folders+'/'+label_dir[i]+'/'+'*.png')
    for j,focus_png in enumerate(focus_pngs):
        img = cv2.imread(focus_png)
        img_predcit =cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img_predcit = np.expand_dims(img_predcit, axis=0)
        img_predcit = img_predcit/255.
        prediction = resnet_model.predict(img_predcit)
        if np.argmax(prediction)<=1:
            save_dir = '/cptjack/totem/yanyiting/Eight_classification/gray/2_focus/train/focus'
        if np.argmax(prediction)>1:
            save_dir = '/cptjack/totem/yanyiting/Eight_classification/gray/2_focus/train/unfocus'
